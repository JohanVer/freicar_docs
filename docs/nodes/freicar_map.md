...
The node also depends on `freicar_common`, a package that defines common messages, services and enums. Add it to your catkin worksace if it's not already there.

## 1.1. Steps
`freicar_map` is a typical ROS node. It should be copied, or better yet sim-linked inside the catkin workspace for compilation. Afterwards, you can compile `freicar_map` like a normal node:
```bash
cd ~/freicar_ws
...
Returns a reference to the static map object. It's advised to create a single accessible reference during the initialization phase of your program to avoid multiple `map::Map::GetInstance()`.
